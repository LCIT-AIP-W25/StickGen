...
candidate_labels = ["Politics", "Business", "Technology", "Health", "Sports", "Entertainment", "Science", "Crime", "Education"]
...
def remove_leading_video(text):
    return re.sub(r'^video[:\-]?\s*', '', text, flags=re.IGNORECASE).strip()
...
def scrape_and_process(processed_hashes):
    ...
    for _ in range(3):
        ...
        for item in news_items:
            try:
                raw_headline = item.find('h3').get_text(strip=True) if item.find('h3') else 'No Title'
                headline = remove_leading_video(raw_headline)
    ...
    if articles:
        logging.info(f"Scraped and processed {len(articles)} new articles.")
